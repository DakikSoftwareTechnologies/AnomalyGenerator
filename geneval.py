...
from tensorflow.keras.layers import Dense, Lambda, LSTM, Input, RepeatVector, TimeDistributed
...
class HyperVAE(keras_tuner.HyperModel):
  ...
  def build(self, hp):
    ...
    dense_layer_units = hp.Int('dense_layer_units', min_value=32, max_value=64, step=16)
    ...
    last_layer_units = hp.Int('last_layer_units', min_value=32, max_value=512, step=32)

    lstm_layers = list()
    for i in range(layers):
      lstm_layers.append(hp.Int(f'lstm_{i}_units', min_value=32, max_value=512, step=32))

    model_input = Input(shape=(self.sequence_length, self.input_dim), name="encoder_input")
    enc = LSTM(first_layer_units, name="encoder_lstm_first", 
              return_sequences=True, activation=activation)(model_input)
    for i in range(layers):
        enc = LSTM(lstm_layers[i], return_sequences=True, activation=activation)(enc)
    ...
    for i in range(layers):
      dec = LSTM(lstm_layers[layers - 1 - i], return_sequences=True, activation=activation)(dec)
    ...
    vae.add_loss(vae_loss(model_input, model_output, log_var, mu, self.sequence_length, self.alpha))
...
def add_percentage_anomaly(arr, anomaly_normal_ratio=0.01, noise=0.05, wave=False, noise_direction="p", wave_length=0):
...
## Read csv and return as df
def read_csv_single(path, index=None, delimiter=None, header=None, parse_dates=True, nrows=None):
  if not parse_dates:
    # df = pd.read_csv(path, delimiter=delimiter, header=header, index_col=index, parse_dates=parse_dates)
    df = pd.read_csv(path, delimiter=delimiter, header=header, parse_dates=parse_dates, nrows=nrows)
  elif parse_dates:
    df = pd.read_csv(path, delimiter=delimiter, header=header, parse_dates=parse_dates, nrows=nrows)
...
def read_csv_multi(path, index=None, delimiter=None, parse_dates=False, header=True, nrows=None):
  all_files = glob.glob(path + "*.csv")
  df_list = []
  for f in all_files:
      df = pd.read_csv(f, delimiter=delimiter, index_col=index, parse_dates=parse_dates, nrows=nrows)
...
def create_vae(sequence_length, input_dim, alpha, latent_space_dim, learning_rate, batch_size, epochs, training_data):
  model_input = Input(shape=(sequence_length, input_dim), name="encoder_input")
  enc = LSTM(64, name="encoder_lstm_1", return_sequences=True, activation='tanh')(model_input)
  ...
  model_output = decoder(encoder(model_input))
  vae = Model(model_input, model_output, name="autoencoder")
  vae.add_loss(vae_loss(model_input, model_output, log_var, mu, sequence_length, alpha))
...
def counting_method(target, candidate, range):
  ...
  tdir = (exact_match + detected_anomaly) / (exact_match + detected_anomaly + missed_anomaly)
  dair = (exact_match + detected_anomaly) / (exact_match + detected_anomaly + false_anomaly)
...
def create_autoencoder(sequence_length, input_dim, latent_space_dim, learning_rate):
  model_input = Input(shape=(sequence_length, input_dim), name="encoder_input")
  enc = LSTM(128, name="encoder_lstm_1", return_sequences=True, activation='tanh')(model_input)
  enc = LSTM(64, name="encoder_lstm_2", return_sequences=True, activation='tanh')(enc)
  enc = LSTM(32, name="encoder_lstm_3", return_sequences=False, activation='tanh')(enc)
  ...
  dec = LSTM(128, name="decoder_lstm_3", return_sequences=True, activation='tanh')(dec)
...
def generate_non_vae_anomalies(noise_params, imdir, outdir):
  ...
  if noise_params["csv_mode"] == "single":
    df = read_csv_single(path, header=0, index=train_params["key"], nrows=noise_params["nrows"])
  else:
    df = read_csv_multi(path, index=[noise_params["key"]], header=True, nrows=noise_params["nrows"])
  test_dfs = list()
  ...
  for i, df in enumerate(test_dfs):
      random_ano, random_points = add_percentage_anomaly(df[noise_params["columns"]].values, noise_params["anomaly_normal_ratio"],
                                                      noise_params["noise"], noise_params["wave"], noise_params["noise_direction"], noise_params["wave_length"])
      ...
      for x, col in enumerate(noise_params["columns"]):
        if x != len(noise_params) - 1:
          ...
          header += col
      np.savetxt(outdir + str(i) + "randomNoise.csv", random_ano, delimiter=",", header=header, comments='')
...
def generate_anomalies(train_params, imdir):
  ...
  training_data, testing_data, means, stds = prepare_data(sub_df, used_cols, train_params["sequence_length"], dates)
  training_data = np.asarray(training_data)
  testing_data = np.asarray(testing_data)
  _encoder, decoder, stats_generator, _vae  = create_vae(train_params["sequence_length"],
                                                                      train_params["input_dim"], 100, train_params["latent_space_dim"], train_params["learning_rate"],
                                                                      train_params["batch_size"], train_params["epochs"], np.asarray(training_data[0]).astype('float32'))

  ano_recon, ano_points = test_anomaly(testing_data[0], stats_generator, decoder)
  ano_recon = ano_recon.reshape(ano_recon.shape[0], ano_recon.shape[2], ano_recon.shape[3])
...
def evaluate_model(input):
  ...
  for i, target in enumerate(target_windows):
    tdir, dair, em, da, ma, fa = counting_method(target, candidate_windows[i], input["range"])
...
if __name__ == '__main__':
    parser = ap.ArgumentParser(description='Generate anomalous data or evaluate anomaly detection output')
    parser.add_argument('-t', '--task', help='Task to perform, "ano" or "eval"', type=str, required=True)
    ...
    if args["task"] == "ano":
      ...
      if "noise_params" in input_file.keys():
        ...
        generate_non_vae_anomalies(noise_params, input_file["imdir"], args["outdir"])
        exit()
      ano_df, ano_points = generate_anomalies(train_params, input_file["imdir"], args["outdir"])
